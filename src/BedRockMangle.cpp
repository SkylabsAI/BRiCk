...
/*
 * This file implements a "mangler" the should assign stable names to all types.
    ...
    if (dc == nullptr or dc->isTranslationUnit()) {
        ...
        for (auto i = 0; i < args.size(); ++i) {
            ...
            case TemplateArgument::ArgKind::Declaration:
                ...
                break;
            case TemplateArgument::ArgKind::Integral:
                // TODO the use of [i] here is a bug
                print.output() << "Li" << arg.getAsIntegral() << "E";
                break;
            case TemplateArgument::ArgKind::NullPtr:
                print.output() << "Dn";
                break;
            case TemplateArgument::ArgKind::Type: {
                ...
                if (auto tag = qt.getTypePtr()->getAsTagDecl()) {
                    printTypeName(arg.getAsType().getTypePtr()->getAsTagDecl(),
                                  print, cprint, mangle);
                } else if (auto bt = dyn_cast<BuiltinType>(qt)) {
                    switch (bt->getKind()) {
#define CASE(X, c)                                                             \
...
#undef CASE
                    default:
                        logging::unsupported()
                            << "unsupported mangling for builtin: "
                            << bt->getKind() << "\r\n";
                        print.output() << "?(builtin:" << bt->getKind() << ")";
                    }
                }
                break;
            }
