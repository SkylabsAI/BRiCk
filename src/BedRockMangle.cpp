...
//===----------------------------------------------------------------------===//
//
// Implements C++ name mangling according to the BedRock C++ ABI,
// which is used in GCC 3.2 and newer (and many compilers that are
// ABI-compatible with GCC):
//
//   http://bedrock-cxx-abi.github.io/cxx-abi/abi.html#mangling
//
//===----------------------------------------------------------------------===//
...
public:
    ...
    static std::string tagged(const std::string nm, char prefix = 0) {
        std::string result = llvm::utostr(nm.length() + prefix ? 1 : 0);
        result += prefix;
        result += nm;
        return result;
    }
    ...
    std::string getAnonymousStructId(const NamedDecl *D,
        ...
        if (auto rd = dyn_cast<RecordDecl>(D)) {
            if (auto tnd = rd->getTypedefNameForAnonDecl()) {
                return tagged(tnd->getNameAsString(), '~');
            } else {
                if (auto vd = dyn_cast<ValueDecl>(rd->getNextDeclInContext())) {
                    return tagged(vd->getNameAsString(), '%');
                } else {
                    return "<OTHER>";
                }
            }
...
public:
    ...
    std::string getLambdaString(const CXXRecordDecl *Lambda) override {
        ...
        auto LambdaId = getAnonymousStructIdForDebugInfo(Lambda);
...
BedRockMangleContextImpl::mangleCXXName(GlobalDecl GD, raw_ostream &Out) {
    const NamedDecl *D = cast<NamedDecl>(GD.getDecl());
    assert(
        (isa<FunctionDecl, VarDecl, TemplateParamObjectDecl>(D)) &&
...
ClangPrinter::printObjName(const ValueDecl *decl, CoqPrinter &print, bool raw) {
    ...
    if (auto ecd = dyn_cast<EnumConstantDecl>(decl)) {
        ...
        print.end_ctor();
    } else if (mc->shouldMangleDeclName(decl)) {
        print.output() << "\"";
