...
mlock
Definition Rep_BUpd `{!cpp_logic thread_info Σ} : BUpd RepI :=
  match eq_sym RepI.unlock in _ = X return BUpd (X _ _ _) with
  | eq_refl => _
  end.

mlock
Definition Rep_FUpd `{!cpp_logic thread_info Σ} : FUpd RepI :=
  match eq_sym RepI.unlock in _ = X return FUpd (X _ _ _) with
  | eq_refl => _
  end.
...
#[global] Existing Instances Rep_BUpd Rep_FUpd Rep_InternalEq.
...
Section with_cpp.
  ...
  Proof.
    rewrite /Persistent.
    unwrap_Rep. constructor=>p /=.
    rewrite monPred_at_persistently; apply H.
  Qed.
  ...
  Proof.
    rewrite /Timeless; unwrap_Rep.
    constructor=>p; rewrite /= monPred_at_later monPred_at_except_0. apply H.
  Qed.
  #[global] Instance as_Rep_fractional P :
    (∀ p, Fractional (λ q, P q p)) → Fractional (λ q, as_Rep (P q)).
  Proof.
    rewrite /Fractional; unwrap_Rep.
    constructor=>?. by rewrite monPred_at_sep /= H.
  Qed.
  ...
  Proof. rewrite /CFractional; unwrap_Rep. constructor=>?; by rewrite monPred_at_sep /= H. Qed.
  ...
  Proof.
    rewrite /Laterable; unwrap_Rep; constructor=>?/=.
    iIntros "R". iDestruct (H with "R") as (R') "[R #close]".
    rewrite monPred_at_exist. iExists {| monPred_at := fun _ => R' |}.
  ...
  *)
  ...
  Proof. unwrap_Rep; constructor => ?. by rewrite monPred_at_emp. Qed.
  Lemma as_Rep_sep P Q : as_Rep (λ p, P p ** Q p) -|- as_Rep P ** as_Rep Q.
  Proof. unwrap_Rep; constructor=>p. by rewrite monPred_at_sep. Qed.

  (* NOTE this is not exposed as a hint *)
  Lemma as_Rep_observe P Q (o : forall p, Observe (P p) (Q p)) : Observe (as_Rep P) (as_Rep Q).
  Proof. unwrap_Rep; apply: monPred_observe=>p. Qed.
  Lemma as_Rep_observe_2 (P Q R : ptr -> mpredI) (o : forall p, Observe2 (P p) (Q p) (R p)) :
    Observe2 (as_Rep P) (as_Rep Q) (as_Rep R).
  Proof. unwrap_Rep; apply: monPred_observe_2=>p. Qed.

  #[global] Instance as_Rep_only_provable_observe Q (P : ptr → mpred) :
    (∀ p, Observe [| Q |] (P p)) → Observe [| Q |] (as_Rep P).
  Proof.
    unwrap_Rep; intros. apply monPred_observe=>p. by rewrite monPred_at_only_provable.
  Qed.
  #[global] Instance as_Rep_only_provable_observe_2 Q (P1 P2 : ptr → mpred) :
    (∀ p, Observe2 [| Q |] (P1 p) (P2 p)) →
    Observe2 [| Q |] (as_Rep P1) (as_Rep P2).
  Proof.
    unwrap_Rep; intros. apply monPred_observe_2=>p. by rewrite monPred_at_only_provable.
  Qed.
  ...
  Proof. unwrap_Rep; constructor=>p. by rewrite monPred_at_and. Qed.

  Lemma as_Rep_or P Q : as_Rep (λ p, P p \\// Q p) -|- as_Rep P \\// as_Rep Q.
  Proof. unwrap_Rep; constructor=>p. by rewrite monPred_at_or. Qed.

  Lemma as_Rep_wand P Q : as_Rep (λ p, P p -* Q p) -|- as_Rep P -* as_Rep Q.
  Proof.
    unwrap_Rep; constructor=>p /=. split'.
  ...
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_exist. Qed.

  Lemma as_Rep_forall {T} (P : T -> ptr -> mpred) :
    as_Rep (λ p, Forall x, P x p) -|- Forall x, as_Rep (P x).
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_forall. Qed.

  Lemma as_Rep_pers P : as_Rep (λ p, <pers> P p) -|- <pers> (as_Rep P).
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_persistently. Qed.


  Lemma as_Rep_bupd P : as_Rep (λ p, |==> P p) -|- |==> as_Rep P.
  Proof.
    rewrite !as_Rep.unlock !Rep_BUpd.unlock /Rep;
      generalize dependent RepI.unlock;
      generalize dependent (@RepI.body); intros; subst => /=.
    constructor=>? /=. by rewrite monPred_at_bupd.
  Qed.

  Lemma as_Rep_fupd P E1 E2 : as_Rep (λ p, |={E1,E2}=> P p) -|- |={E1,E2}=> as_Rep P.
  Proof.
    rewrite !as_Rep.unlock !Rep_FUpd.unlock /Rep;
      generalize dependent RepI.unlock;
      generalize dependent (@RepI.body); intros; subst => /=.
    constructor=>? /=. by rewrite monPred_at_fupd.
  Qed.

  Lemma as_Rep_intuitionistically P : as_Rep (λ p, □ P p) -|- □ as_Rep P.
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_intuitionistically. Qed.

  Lemma as_Rep_intuitionistically_if b P : as_Rep (λ p, □?b P p) -|- □?b as_Rep P.
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_intuitionistically_if. Qed.

  Lemma as_Rep_except_0 P : as_Rep (λ p, bi_except_0 (P p)) -|- bi_except_0 (as_Rep P).
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_except_0. Qed.

  Lemma as_Rep_affinely P : as_Rep (λ p, <affine> P p) -|- <affine> (as_Rep P).
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_affinely. Qed.

  Lemma as_Rep_affinely_if b P : as_Rep (λ p, <affine>?b P p) -|- <affine>?b (as_Rep P).
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_affinely_if. Qed.

  Lemma as_Rep_big_sepL {T} (l : list T) (F : nat -> T -> ptr -> mpred) :
    as_Rep (λ p, [∗list] i ↦ x ∈ l, F i x p) -|- [∗list] i ↦ x ∈ l, as_Rep (F i x).
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_big_sepL. Qed.

  Lemma as_Rep_later P : as_Rep (λ p, |> P p) -|- |> as_Rep P.
  Proof. unwrap_Rep; constructor=>p /=. by rewrite monPred_at_later. Qed.


  Lemma as_Rep_internal_eq (P Q : mpred) : as_Rep (λ _, P ≡ Q) -|- P ≡ Q.
  Proof.
    rewrite !as_Rep.unlock Rep_InternalEq.unlock /Rep;
      generalize dependent RepI.unlock;
      generalize dependent (@RepI.body); intros; subst => /=.
    constructor=>?.
    by rewrite monPred_at_internal_eq.
  Qed.
  ...
  (*   _at p (P ∗-∗ Q) ⊣⊢ (_at p P ∗-∗ _at p Q). *)
  (* Proof. rewrite /bi_wand_iff. monPred_at_and !Rep_wand_force. Qed. *)
  ...
  Proof.
    rewrite __at.unlock /Rep.
    rewrite /AT_at. rewrite /offsetA offsetR_aux.unlock !as_Rep.unlock !at_aux.unlock /Rep /=.
    generalize dependent RepI.unlock.
    generalize dependent (@RepI.body). intros; subst; simpl.
  ...
  Ltac unwrap_Rep_offsetR :=
    rewrite __at.unlock /Rep /AT_at /offsetA !offsetR_aux.unlock !as_Rep.unlock !at_aux.unlock /Rep /=;
      generalize dependent RepI.unlock; simpl;
    generalize dependent (@RepI.body); intros; subst; simpl;
    constructor=>p /=.
  ...
  Proof.
    revert r; rewrite /Persistent; unwrap_Rep_offsetR.
    destruct H as [->]. rewrite !monPred_at_persistently; done.
  Qed.
  #[global] Instance _offsetR_affine o (r : Rep) : Affine r -> Affine (o |-> r).
  Proof.
    revert r; rewrite /Affine; unwrap_Rep_offsetR.
    destruct H as [->]. rewrite !monPred_at_emp; done.
  Qed.
  #[global] Instance _offsetR_timeless o (r : Rep) : Timeless r → Timeless (o |-> r).
  Proof.
    revert r; rewrite /Timeless; unwrap_Rep_offsetR.
    rewrite /= monPred_at_later monPred_at_except_0 /= -monPred_at_later -monPred_at_except_0.
    apply H.
  Qed.
  #[global] Instance _offsetR_laterable o (r : Rep) : Laterable r → Laterable (o |-> r).
  Proof.
    revert r; rewrite /Laterable; unwrap_Rep_offsetR.
  Admitted.
  ...
  Proof. (* rewrite !_offsetR_eq. by constructor=> p/=; rewrite !monPred_at_persistently. Qed. *) Admitted.

  Lemma _offsetR_bupd o R : o |-> (|==> R) -|- |==> o |-> R.
  Proof. (* by rewrite !_offsetR_eq /as_Rep; constructor => p /=; rewrite !monPred_at_bupd. Qed. *) Admitted.

  Lemma _offsetR_fupd o R E1 E2 : o |-> (|={E1,E2}=> R) -|- |={E1,E2}=> o |-> R.
  Proof. (* by rewrite !_offsetR_eq /as_Rep; constructor => p /=; rewrite !monPred_at_fupd. Qed. *) Admitted.

  Lemma _offsetR_intuitionistically o (R : Rep) : o |-> (□ R) ⊣⊢ □ (o |-> R).
  Proof. (* by rewrite !_offsetR_eq; constructor => p /=; rewrite !monPred_at_intuitionistically. Qed. *) Admitted.

  Lemma _offsetR_intuitionistically_if o b R : o |-> (□?b R) -|- □?b (o |-> R).
  Proof. (* by destruct b => //=; rewrite /= _offsetR_intuitionistically. Qed. *) Admitted.

  Lemma _offsetR_except_0 o R : o |-> (bi_except_0 R) -|- bi_except_0 (o |-> R).
  Proof. (* by rewrite !_offsetR_eq; constructor => p /=; rewrite !monPred_at_except_0. Qed. *) Admitted.

  Lemma _offsetR_affinely (o : offset) R : o |-> (<affine> R) -|- <affine> o |-> R.
  Proof. (* by rewrite !_offsetR_eq /as_Rep; constructor => p/=; rewrite !monPred_at_affinely. Qed. *) Admitted.

  Lemma _offsetR_affinely_if b (o : offset) R : o |-> (<affine>?b R) -|- <affine>?b o |-> R.
  Proof. (* by destruct b => //; rewrite _offsetR_affinely. Qed. *) Admitted.
  ...
  Proof. (*
    rewrite !_offsetR_eq. unwrap_Rep; constructor=>p/=. by rewrite !monPred_at_later.
  Qed. *) Admitted.

  #[global] Instance _offsetR_fractional o (r : Qp → Rep) :
    Fractional r → Fractional (λ q, o |-> r q).
  Proof. (* by move => H q1 q2; rewrite fractional _offsetR_sep. Qed. *) Admitted.
  #[global] Instance _offsetR_as_fractional o (R : Rep) (r : Qp → Rep) q
      `{!AsFractional R r q} :
    AsFractional (o |-> R) (λ q, o |-> r q) q.
  Proof. (* constructor; [by erewrite as_fractional | apply _]. Qed. *) Admitted.

  #[global] Instance _offsetR_cfractional {R : cQp.t -> Rep} o :
    CFractional R ->
    CFractional (fun q => _offsetR o (R q)).
  Proof. (* intros HR q1 q2. constructor =>p. by rewrite cfractional _offsetR_sep. Qed. *) Admitted.
  #[global] Instance _offsetR_as_cfractional o (P : Rep) (R : cQp.t -> Rep) q :
    AsCFractional P R q ->
    AsCFractional (_offsetR o P) (λ q, _offsetR o (R q)) q.
  Proof. (* constructor. by rewrite -as_cfractional. apply _. Qed. *) Admitted.

  #[global] Instance _offsetR_observe {o} {Q R : Rep} :
    Observe Q R ->
    Observe (o |-> Q) (o |-> R).
  Proof. (* move->. by rewrite /Observe -_offsetR_pers. Qed. *) Admitted.
  ...
  Proof. (*
    rewrite /Observe2.
    move->. by rewrite /Observe2 _offsetR_wand _offsetR_pers. Qed. *) Admitted.

  #[global] Instance _offsetR_observe_only_provable Q o (R : Rep) :
    Observe [| Q |] R → Observe [| Q |] (o |-> R).
  Proof. (* rewrite -{2}_offsetR_only_provable. apply _. Qed. *) Admitted.
  ...
  #[global] Instance _offsetR_observe_pure Q o (R : Rep) :
    Observe [! Q !] R → Observe [! Q !] (o |-> R).
  Proof. (* rewrite -{2}_offsetR_pure. apply _. Qed. *) Admitted.
  ...
  Proof. (* unfold_at. solve_proper. Qed. *) Admitted.
  ...
  Proof. (* unfold_at; move => r1 r2 HR/=. solve_proper. Qed. *) Admitted.

  #[global] Instance _at_persistent {P base} : Persistent P -> Persistent (_at base P).
  Proof. (* rewrite _at_eq. apply _. Qed. *) Admitted.
  ...
  Proof. (* by rewrite _at_eq. Qed. *) Admitted.
  ...
  Proof. (* by rewrite _at_loc monPred_at_emp. Qed. *) Admitted.

  Lemma _at_exists p {T} (P : T -> Rep) :
      p |-> (Exists v : T, P v) -|- Exists v, p |-> (P v).
  Proof. (* by rewrite _at_loc monPred_at_exist; setoid_rewrite _at_loc. Qed. *) Admitted.

  Lemma _at_forall p T (P : T -> Rep) :
    p |-> (Forall x, P x) -|- Forall x, p |-> (P x).
  Proof. (* by rewrite _at_loc monPred_at_forall; setoid_rewrite _at_loc. Qed. *) Admitted.

  Lemma _at_only_provable p P :
      p |-> [| P |] -|- [| P |].
  Proof. (* by rewrite _at_loc monPred_at_only_provable. Qed. *) Admitted.

  Lemma _at_pure p (P : Prop) :
      p |-> [! P !] -|- [! P !].
  Proof. (* by rewrite _at_loc monPred_at_pure. Qed. *) Admitted.

  Lemma _at_sep p (P Q : Rep) :
      p |-> (P ** Q) -|- p |-> P ** p |-> Q.
  Proof. (* by rewrite !_at_loc monPred_at_sep. Qed. *) Admitted.

  Lemma _at_and p (P Q : Rep) :
      p |-> (P //\\ Q) -|- p |-> P //\\ p |-> Q.
  Proof. (* by rewrite !_at_loc monPred_at_and. Qed. *) Admitted.

  Lemma _at_or p (P Q : Rep) :
      p |-> (P \\// Q) -|- p |-> P \\// p |-> Q.
  Proof. (* by rewrite !_at_loc monPred_at_or. Qed. *) Admitted.

  Lemma _at_wand p (P Q : Rep) :
      p |-> (P -* Q) -|- p |-> P -* p |-> Q.
  Proof. (* by rewrite !_at_loc Rep_wand_force. Qed. *) Admitted.

  Lemma _at_impl p R1 R2 : p |-> (R1 -->> R2) -|- p |-> R1 -->> p |-> R2.
  Proof. (* by rewrite !_at_loc Rep_impl_force. Qed. *) Admitted.

  Lemma _at_pers p R : p |-> (<pers> R) -|- <pers> p |-> R.
  Proof. (* by rewrite !_at_loc monPred_at_persistently. Qed. *) Admitted.

  Lemma _at_bupd p R : p |-> (|==> R) -|- |==> p |-> R.
  Proof. (* by rewrite !_at_loc monPred_at_bupd. Qed. *) Admitted.

  Lemma _at_fupd p R E1 E2 : p |-> (|={E1,E2}=> R) -|- |={E1,E2}=> p |-> R.
  Proof. (* by rewrite !_at_loc monPred_at_fupd. Qed. *) Admitted.

  Lemma _at_intuitionistically p (R : Rep) : p |-> □ R ⊣⊢ □ (p |-> R).
  Proof. (* by rewrite !_at_eq; rewrite monPred_at_intuitionistically. Qed. *) Admitted.
  Lemma _at_intuitionistically_if (p : ptr) b R : p |-> (□?b R) -|- □?b (p |-> R).
  Proof. (* destruct b => //=. by rewrite _at_intuitionistically. Qed. *) Admitted.

  Lemma _at_except_0 (p : ptr) R : p |-> bi_except_0 R -|- bi_except_0 (p |-> R).
  Proof. (* by rewrite !_at_eq monPred_at_except_0. Qed. *) Admitted.

  Lemma _at_affinely (p : ptr) R : p |-> <affine> R -|- <affine> p |-> R.
  Proof. (* by rewrite !_at_eq monPred_at_affinely. Qed. *) Admitted.

  Lemma _at_affinely_if b (p : ptr) R : p |-> <affine>?b R -|- <affine>?b p |-> R.
  Proof. (* by destruct b => //; rewrite !_at_eq monPred_at_affinely. Qed. *) Admitted.

  Lemma _at_later p R : p |-> |> R -|- |> p |-> R.
  Proof. (* by rewrite !_at_eq; rewrite monPred_at_later. Qed. *) Admitted.

  Lemma _at_internal_eq p {A : ofe} x y : p |-> (x ≡@{A} y) -|- x ≡ y.
  Proof. (* by rewrite _at_loc monPred_at_internal_eq. Qed. *) Admitted.

  Lemma _at_offsetR p (o : offset) (r : Rep) :
      _at p (_offsetR o r) -|- _at (p ,, o) r.
  Proof. (* rewrite !_at_loc /flip. by unfold_at. Qed. *) Admitted.
  ...
  Proof. (* by rewrite _at_big_sepL big_sepL_fmap. Qed. *) Admitted.
  ...
  Proof. (* intros HR q1 q2. by rewrite cfractional _at_sep. Qed. *) Admitted.
  ...
  #[global] Instance _at_observe {p} {Q R : Rep} :
    Observe Q R ->
    Observe (p |-> Q) (p |-> R).
  Proof. (* move->. by rewrite /Observe _at_pers. Qed. *) Admitted.

  #[global] Instance _at_observe_2 {p} {Q R1 R2 : Rep} :
    Observe2 Q R1 R2 ->
    Observe2 (p |-> Q) (p |-> R1) (p |-> R2).
  Proof. (* move->. by rewrite /Observe2 _at_wand _at_pers. Qed. *) Admitted.

  #[global] Instance _at_observe_only_provable Q p (R : Rep) :
    Observe [| Q |] R → Observe [| Q |] (p |-> R).
  Proof. (* rewrite -_at_only_provable. apply _. Qed. *) Admitted.
  ...
  #[global] Instance _at_observe_pure Q p (R : Rep) :
    Observe [! Q !] R → Observe [! Q !] (p |-> R).
  Proof. (* rewrite -_at_pure. apply _. Qed. *) Admitted.
  ...
  Proof. (* rewrite !_at_loc. apply _. Qed. *) Admitted.
  ...
  Proof. (* by move=>?; rewrite cfractional_list_div // _at_big_sepL. Qed. *) Admitted.

  Lemma _at_obs p (r : Rep) P :
    r |-- r ** [| P |] →
    p |-> r |-- p |-> r ** [| P |].
  Proof. (* intros. apply observe_elim, _at_observe_only_provable. exact: observe_intro. Qed. *) Admitted.
  ...
  Proof. (* by constructor. Qed. *) Admitted.
  ...
  Proof. (* intros [??]. constructor. done. apply _. Qed. *) Admitted.
  ...
  (* Proof. (* done. Qed. *) Admitted. *)

  #[global] Instance pureR_laterable P : Laterable P → Laterable (pureR P).
  Proof. (* intros. exact: as_Rep_laterable. Qed. *) Admitted.

  Lemma pureR_persistently P : pureR (<pers> P) -|- <pers> pureR P.
  Proof. (* constructor=>p /=. by rewrite monPred_at_persistently. Qed. *) Admitted.
  ...
  Lemma pureR_obs P Q :
    P |-- P ** [| Q |] →
    pureR P |-- pureR P ** [| Q |].
  Proof. (* intros. apply observe_elim, pureR_observe. exact: observe_intro. Qed. *) Admitted.
  ...
  Proof. (* by rewrite !_at_loc /pureR. Qed. *) Admitted.

  Lemma _offsetR_pureR o P : o |-> (pureR P) -|- pureR P.
  Proof. (* by apply Rep_equiv_at => p; rewrite _at_offsetR !_at_pureR. Qed. *) Admitted.
  ...
  Section o_sub.
    ...
    Lemma _offsetR_sub_0 ty R
      ...
      _offsetR (.[ ty ! 0 ]) R -|- R.
    Proof. (* by rewrite o_sub_0 // _offsetR_id. Qed. *) Admitted.

    Lemma _offsetR_sub_sub ty a b R :
      .[ ty ! a ] |-> (.[ ty ! b ] |-> R) ⊣⊢
      .[ ty ! a + b ] |-> R.
    Proof. (* by rewrite _offsetR_dot o_dot_sub. Qed. *) Admitted.

    Lemma _offsetR_succ_sub ty z R :
      .[ ty ! 1 ] |-> (.[ ty ! z ] |-> R) ⊣⊢
      .[ ty ! Z.succ z] |-> R.
    Proof. (* by rewrite _offsetR_sub_sub Z.add_1_l. Qed. *) Admitted.

    Lemma _offsetR_sub_succ ty z R :
      .[ ty ! z ] |-> (.[ ty ! 1 ] |-> R) ⊣⊢
      .[ ty ! Z.succ z] |-> R.
    Proof. (* by rewrite _offsetR_sub_sub Z.add_1_r. Qed. *) Admitted.

    Lemma _at_sub_0 p ty R
      (Hsz : is_Some (size_of σ ty)) :
      p .[ ty ! 0 ] |-> R -|- p |-> R.
    Proof. (* by rewrite offset_ptr_sub_0. Qed. *) Admitted.

    Lemma _at_sub_sub p ty a b R :
      p .[ ty ! a ] |-> (.[ ty ! b ] |-> R) ⊣⊢
      p .[ ty ! a + b ] |-> R.
    Proof. (* by rewrite -!_at_offsetR _offsetR_sub_sub. Qed. *) Admitted.

    Lemma _at_succ_sub p ty z R :
      p .[ ty ! 1 ] |-> (.[ ty ! z ] |-> R) ⊣⊢
      p .[ ty ! Z.succ z] |-> R.
    Proof. (* by rewrite -!_at_offsetR _offsetR_succ_sub. Qed. *) Admitted.

    Lemma _at_sub_succ p ty z R :
      p .[ ty ! z ] |-> .[ ty ! 1 ] |-> R ⊣⊢
      p .[ ty ! Z.succ z] |-> R.
    Proof. (* by rewrite -!_at_offsetR _offsetR_sub_succ. Qed. *) Admitted.
  End o_sub.
End with_cpp.
