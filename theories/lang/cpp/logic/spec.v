...
Section with_prop.
  ...
  Definition WithExG@{R} {RESULT : Type@{R}} :=
    (RESULT -> PROP) -> PROP.
  Global Arguments WithExG : clear implicits.

  Definition WithPrePostG@{A R}  {ARGS : Type@{A}} {RESULT : Type@{R}} :=
    ARGS -> (RESULT -> PROP) -> PROP.
(*
  Record WithPrePostG@{X Z Y A R} {ARGS : Type@{A}} {RESULT : Type@{R}} :=
    { wpp_with : tele@{X}
    ; wpp_pre  : tele_fun@{X Z Y} wpp_with (ARGS * PROP)
    ; wpp_post : tele_fun@{X Z Y} wpp_with (WithExG@{_} RESULT)}.
  Global Arguments WithPrePostG : clear implicits.
 *)

  (** Mnemonic: WppGD stands for "[WithPrePostG]'s denotation" *)
  Definition WppGD@{A R} {ARGS RESULT} (wpp : @WithPrePostG@{A R} ARGS RESULT) (params : ARGS)
             (Q : RESULT -> PROP) : PROP :=
    wpp params Q.
  (*
  refine (
    tbi_exist@{X Z Y} (tele_bind (TT:=wpp.(wpp_with)) (fun args =>
      ...
      [| params = fst P |] ** snd P ** tele_app wpp.(wpp_post) args Q))).
  Show Proof.
 ...
 *)
  ...
  arguments. *)
  Definition WithEx := WithExG@{Set} val.
  Definition WithPrePost := @WithPrePostG@{Set Set} (list val) val.
  Definition WppD := (WppGD (ARGS:=list val) (RESULT:=val)).
