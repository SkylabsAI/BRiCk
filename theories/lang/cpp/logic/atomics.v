...
Section with_Σ.
  ...
  Axiom wp_atom_store_cst :
    forall memorder acc_type p Q v,
      [| memorder = _SEQ_CST |] **
      [| has_type v acc_type |] **
      wrap_shift (fun Q => _eqv p |-> anyR acc_type (CV.mut 1) ** (* pre *)
                          (_eqv p |-> primR acc_type (CV.mut 1) v -* Q Vundef)) Q (* post *)
  ...
  Axiom wp_atom_exchange_n_cst :
    forall memorder acc_type p Q v,
      ...
      wrap_shift (fun Q => Exists w,
                          _eqv p |-> primR acc_type (CV.mut 1) w ** (* pre *)
                          (_eqv p |-> primR acc_type (CV.mut 1) v -* Q w)) Q (* post *)
  ...
  Axiom wp_atom_exchange_cst :
    forall memorder acc_type p Q new_p q ret new_v,
      ...
      |> ((* local pre-cond *)
          ...
          _eqv ret |-> anyR acc_type (CV.mut 1)) **
      AU1 <<∀ v, (* atomic pre-cond: latest value of p is v *)
              _eqv p |-> primR acc_type (CV.mut 1) v >> @M,∅
          ...
          <<  (* atomic post-cond: latest value updated to new_v *)
              _eqv p |-> primR acc_type (CV.mut 1) new_v,
            COMM ((* post-cond: the client can assume the local points-to, which
                  ...
                  _eqv ret |-> primR acc_type (CV.mut 1) v -* Q v) >>
  ...
  Axiom wp_atom_compare_exchange_n_cst :
    ...
    let ty := Tnum sz sgn in
      ...
      |> _eqv expected_p |-> primR ty (CV.mut 1) (Vint expected_v) **
      AU1 <<∀ v, (* atomic pre-cond: latest value of p is v *)
              _eqv p |-> primR ty (CV.mut 1) (Vint v) >> @M,∅
          <<∃ (b : bool) (v' : Z), (* atomic post-cond: latest value is v' *)
              _eqv p |-> primR ty (CV.mut 1) (Vint v') **
            ...
            COMM (* post-cond *)
                _eqv expected_p |-> primR ty (CV.mut 1) (Vint v) -* Q (Vbool b) >>
  ...
  Axiom wp_atom_compare_exchange_cst :
    forall q p expected_p desired_p weak succmemord failmemord Q
      ...
      [| succmemord = _SEQ_CST |] ** [| failmemord = _SEQ_CST |] **
      |> ((* local pre-cond *)
          _eqv expected_p |-> primR ty (CV.mut 1) (Vint expected) **
      ...
      AU1 <<∀ v, (* atomic pre-cond: latest value of p is v *)
          ...
          <<∃ (b : bool) v', (* atomic post-cond: latest value is v' *)
            ...
            COMM ((* post-cond *)
                  _eqv expected_p |-> primR ty (CV.mut 1) (Vint v) **
  ...
  Definition wp_fetch_xxx_cst (ao : AtomicOp) (op : Z -> Z -> Z) : Prop :=
    forall p arg memorder Q sz sgn,
      ...
      wrap_shift (fun Q =>
                    Exists n,
                    _eqv p |-> primR acc_type (CV.mut 1) (Vint n) **
                    (let n' := at_eval sz sgn op n arg in
                      _eqv p |-> primR acc_type (CV.mut 1) (Vint n') -* Q (Vint n))) Q
  ...
  Definition atom_store_cst_AU1 (ty : type) (p : val) (Q : val -> mpred) v : mpred :=
    AU1 << ▷ _eqv p |-> anyR ty (CV.mut 1) >> @M,∅
        << ▷ _eqv p |-> primR ty (CV.mut 1) v,
            COMM Q Vundef >>.
  ...
  Definition atom_exchange_n_cst_AU1 (ty : type) (p : val) (Q : val -> mpred) v : mpred :=
    AU1 <<∀ w, ▷ _eqv p |-> primR ty (CV.mut 1) w >> @M,∅
        <<     ▷ _eqv p |-> primR ty (CV.mut 1) v,
            COMM Q w >>.
  ...
  Definition atom_fetch_xxx_cst_AU1 (op : Z -> Z -> Z)
    ...
    AU1 <<∀ n, ▷ _eqv p |-> primR ty (CV.mut 1) (Vint n) >> @M,∅
        <<     let n' := at_eval sz sgn op n z in
              ▷ _eqv p |-> primR ty (CV.mut 1) (Vint n'),
            COMM Q (Vint n) >>.
  ...
  Definition atom_xxx_fetch_cst_AU1 (op : Z -> Z -> Z)
    ...
    AU1 <<∀ n (n' := at_eval sz sgn op n z),
              ▷ _eqv p |-> primR ty (CV.mut 1) (Vint n) >> @M,∅
        <<     ▷ _eqv p |-> primR ty (CV.mut 1) (Vint n'),
            COMM Q (Vint n') >>.
