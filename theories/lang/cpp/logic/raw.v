...
Axiom decode_uint_anyR : ∀ `{Σ : cpp_logic, σ : genv} q sz,
  ...
  anyR (Tarray Tbyte (int_type.bytesN sz)) q ** type_ptrR (Tnum sz Unsigned).
(* ^^ should be proven *)
...
Axiom raw_byte_of_int_eq : ∀ {σ : genv} sz x rs,
  ...
  ∃ l, decodes_uint l x /\ raw_int_byte <$> l = rs /\ length l = N.to_nat (int_type.bytesN sz).
  ...
  Lemma decode_uint_primR q sz (x : Z) :
    primR (Tnum sz Unsigned) q (Vint x) -|-
      ...
      *)
      [| decodes_uint l x |] **
      [| raw_int_byte <$> l = rs |] **
      [| length l = N.to_nat (int_type.bytesN sz) |] **
  ...
  Section with_Σ.
    ...
    Lemma decodes_uint_to_end :
      forall endianness sz l v,
        ...
        decodes_uint l (to_end endianness (int_type.bitsize sz) v).
    Proof.
      ...
      { by destruct sz. }
      { rewrite rev_length. rewrite Hsz; clear; by destruct sz. }
    Qed.

    Lemma decodes_Z_to_bytes_Unsigned:
      forall sz (n : nat)  (endianness : endian) (z : Z),
        (N.to_nat (int_type.bytesN sz) = n)%nat ->
    ...
    Proof.
      ...
      2: {
        ...
        destruct sz; rewrite/int_type.bytesN/=; split; lia.
      }
    ...
    Lemma raw_bytes_of_val_to_end_raw_int_byte (endianness : endian) sz (z : Z) :
      has_type_prop z (Tnum sz Unsigned) ->
      raw_bytes_of_val σ (Tnum sz Unsigned) (to_end endianness (int_type.bitsize sz) z)
        (map raw_int_byte (_Z_to_bytes (int_type.bytesNat sz) endianness Unsigned z)).
    Proof.
      rewrite raw_byte_of_int_eq.
      exists (_Z_to_bytes (int_type.bytesNat sz) endianness Unsigned z).
      intuition.
    ...
    Lemma raw_bytes_of_val_to_big_end_raw_int_byte sz (z : Z) :
      has_type_prop z (Tnum sz Unsigned) ->
      raw_bytes_of_val σ (Tnum sz Unsigned) (to_big_end (int_type.bitsize sz) z)
        (map raw_int_byte (_Z_to_bytes (int_type.bytesNat sz) Big Unsigned z)).
    Proof.
    ...
    Lemma raw_bytes_of_val_to_little_end_raw_int_byte sz (z : Z) :
      has_type_prop z (Tnum sz Unsigned) ->
      raw_bytes_of_val σ (Tnum sz Unsigned) (to_little_end (int_type.bitsize sz) z)
        (map raw_int_byte (_Z_to_bytes (int_type.bytesNat sz) Little Unsigned z)).
    Proof.
