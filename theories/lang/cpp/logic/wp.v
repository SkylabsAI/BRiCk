...
Section Kpred.
  ...
  Definition Kreturn {σ : genv} (P : ptr -> mpred) : KpredI :=
    KP (funI rt =>
          match rt with
          | Normal | ReturnVoid => Forall p : ptr, p |-> primR Tvoid (CV.mut 1) Vvoid -* P p
          | ReturnVal p => P p
          | _ => False
          end).
...
Section with_cpp.
  ...
  Axiom wp_operand_wp_init : forall {σ : genv} tu ρ ty addr e Q,
      is_value_type ty ->
      wp_operand tu ρ e (fun v frees => _at addr (primR ty (CV.mut 1) v) -* Q ty frees)
  ...
  (** This is justified in the logic but technically not sactioned by the standard
   ...
   Axiom wp_init_wp_operand : forall {σ : genv} M ρ addr e Q (ty := type_of e),
      ...
      wp_prval M ρ e (fun p free frees =>
         ...
         ∃ v, _at addr (primR ty (CV.mut 1) v) ** Q v frees)
    |-- wp_operand M ρ e Q.
    ]]
   *)
