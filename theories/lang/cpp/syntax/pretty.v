...
Section with_lang.
  ...
  Section atomic_name.
    ...
    Definition printAN (an : atomic_name_ type) : bs :=
      ...
      | Nfunction quals nm args =>
          printFN nm ++ (parens $ sepBy ", " $ printType <$> args) ++ sepBy " " (printFQ <$> quals)
      | Nclosure types => parens $ sepBy ", " $ printType <$> types
      | Nanon n => "#" ++ showN n
  ...
  Fixpoint printN (nm : name) : bs :=
    ...
    | Ninst base i =>
        ...
        in
        printN base ++ angles (sepBy ", " $ printTA <$> i)
  ...
  with printT (ty : type) : bs :=
    ...
    | Tnum int_type.S128 Signed => "int128_t"
    | Tnum int_type.S128 Unsigned => "uint128_t"
    | Twchar => "wchar_t"
    | Tchar8 => "char8_t"
    | Tchar16 => "char16_t"
    | Tchar32 => "char32_t"
    ...
    | Tfunction ft =>
        ...
        (parens $ sepBy ", " $ printT <$> ft.(ft_params))
    | Tarch _ note => "?" ++ note
