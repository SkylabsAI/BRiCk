    ...
    Section with_genv.
      ...
      Definition pure_encodes (t : type) (v : val) (vs : list runtime_val) : Prop :=
        ...
        | Tnum sz sgn =>
          match v with
          | Vint v =>
            in_Z_to_bytes_bounds (int_type.bitsize sz) sgn v /\
            vs = Z_to_bytes (int_type.bitsize sz) sgn v
          | Vundef => pure_encodes_undef (int_type.bitsize sz) vs
          | _ => False
          end
      ...
      Lemma length_encodes t v vs :
        pure_encodes t v vs ->
          length vs = match erase_qualifiers t with
                      | Tbool => 1
                      | Tnum sz _ => int_type.bytesNat sz
      ...
      Proof.
        ...
        destruct (erase_qualifiers _) => //;
          destruct v => //; destruct_and? => //; simpl;
          repeat case_decide => //;
           simplify_eq; eauto.
        { destruct sz; simpl; rewrite /Z_to_bytes/=_Z_to_bytes_eq/_Z_to_bytes_def/=;
                                by case_match. }
        erewrite length_pure_encodes_undef; eauto.
        clear. by destruct sz.
      Qed.
      ...
      Proof.
        ...
        all: try by destruct POINTER_BITSZ; simpl; lia.
        by destruct sz; simpl; lia.
      Qed.
