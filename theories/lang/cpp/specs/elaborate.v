  ...
  Fixpoint elaborate (ret : type) (ts : list type) (ar : function_arity) (args : list val) (wpp : WpSpec_cpp_val) : WpSpec mpredI ptr ptr :=
    ...
    | nil =>
        let finish args :=
          ...
          | inr t =>
              wp_spec_bind wpp args (fun rv => WITH (fun pr : ptr => DONE pr (_at pr (primR t (CV.mut 1) rv))))
          end
        in
        match ar with
    ...
    | t :: ts =>
        ...
        | inr t =>
            add_with (fun pv : ptr => add_with (fun v : val => add_arg pv (
                                           add_pre (_at pv (primR t (CV.mut 1) v)) (add_post (Exists v, _at pv (primR t (CV.mut 1) v))
