 ...
 *)
(**
   Semantics of type promotions and conversions.

   TODO [conv_int] should be in this file.
 *)
...
(*  BEGIN UPSTREAM *)
#[global] Instance N_le_dec : RelDecision N.le :=
  fun x y =>
    match (x <=? y)%N as X return (x <=? y)%N = X -> _ with
    | true => fun pf => left (proj1 (@N.leb_le _ _) pf)
    | false => fun pf => right (proj1 (@N.leb_nle _ _) pf)
    end eq_refl.

#[global] Instance N_lt_dec : RelDecision N.lt :=
  fun x y =>
    match (x <? y)%N as X return (x <? y)%N = X -> _ with
    | true => fun pf => left (proj1 (@N.ltb_lt _ _) pf)
    | false => fun pf => right (proj1 (@N.ltb_nlt _ _) pf)
    end eq_refl.
(* END UPSTREAM *)

(* BEGIN UPSTREAM C++ specific *)
Definition integral_le (a b : bitsize) : Prop :=
  (* the use of [bitsN] is temporary *)
  (bitsN a <= bitsN b)%N.

(* TODO: this is testing the "rank"
   <https://eel.is/c++draft/conv.rank>
 *)
Definition integral_max (a b : bitsize) : bitsize :=
  match a , b with
  | W8 , _ => b
  | _ , W8 => a
  | W16 , _ => b
...
#[global] Instance integral_le_dec : RelDecision integral_le.
Proof. intros ??; apply N_le_dec. Defined.

(* TODO standard citation *)
...
(* END UPSTREAM C++ specific *)


(**
   C++ Defines various notions of conversions between integral types in
   order to perform arithmetic operations correctly. These include,
   - Arithmetic promotion
   - Integral promotion
 *)

(** [underlying_type tu ty] is the type that underlies [ty].
    This is the identity function if [ty] is not an enum.
    In addition to e.g. if
    [ty] is a [Tenum nm], then the underlying type is the underlying
    type of the `enum`.
 *)
Definition underlying_type (tu : translation_unit) (ty : type) : type :=
  let '(cv, ty') := decompose_type ty in
  match ty' with
  | Tenum nm as ty =>
      match tu.(globals) !! nm with
      | Some (Genum ty _) => tqualified cv ty
      | _ => ty
      end
  | _ => ty
  end.

Succeed Example underlying_int : forall tu, underlying_type tu Tint = Tint :=
  ltac:(compute; auto).
...
Section representable.
  ...
  Definition fully_representable (a b : type) : bool :=
    let to_equiv ty :=
      ...
      | Tchar_ ct => Some $ equivalent_char_type _ ct
      | _ => None
      end
    in
...
Definition promote_integral {σ : genv} (tu : translation_unit) (ty : type) : option type :=
  match underlying_type tu ty with
...
Definition promote_arith (ty1 ty2 : type) : option type :=
  ...
  | Tnum sz1 sgn1 , Tnum sz2 sgn2 =>
      if bool_decide (sgn1 = sgn2) then
        Some $ Tnum (integral_max sz1 sz2) sgn1
      else
        ...
        in
        if bool_decide (integral_le ssz usz) then
          Some $ Tnum usz Unsigned
        else if bool_decide (integral_le usz ssz) then
