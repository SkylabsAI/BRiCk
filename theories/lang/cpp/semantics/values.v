...
(* Module Type VALUES_DEFS (MP : PTRS_INTF).
  ...
  Module V. Include VAL_MIXIN with Module P := MP with Module R := R. End V.
  Module RD. Include RAW_BYTES_VAL with Module V := V. End RD.
  Module HT. Include HAS_TYPE with Module V := V. End HT.
End VALUES_DEFS. *)
(*
Module VALUES_DEFS' (MP : PTRS_INTF).
  Include RAW_BYTES.
  Include VAL_MIXIN with Module P := MP
  with Module R := VALUES_DEFS'
  .
  Include RAW_BYTES_VAL.
  Fail Include HAS_TYPE with Module V := V.
End VALUES_DEFS'. *)

  (* Include RAW_BYTES.
  Include VAL_MIXIN with Module P := MP. with Module R := VALUES_DEFS.
  Include RAW_BYTES_VAL with Module P := MP.
  Include HAS_TYPE with Module V := V. *)
(* RAW_BYTES <+ VAL_MIXIN P <+ RAW_BYTES_VAL P <+ *)

(* Module Type VALUES_DEFS (MP : PTRS_INTF).
  Module V. Include VAL_MIXIN with Module P := MP. End V.
  Module RD. Include RAW_BYTES_VAL with Module V := V. End RD.
  Module HT. Include HAS_TYPE with Module V := V. End HT.
End VALUES_DEFS. *)
Module Type VALUES_DEFS (MP : PTRS_INTF).
...
Module Type VALUES_INTF_FUNCTOR (MP : PTRS_INTF).
  Include VALUES_DEFS MP.
  Module RAW_BYTES'.
    Include RAW_BYTES_MIXIN with Module RAW_BYTES_VAL := RAW_BYTES_VAL.
  End RAW_BYTES'.
