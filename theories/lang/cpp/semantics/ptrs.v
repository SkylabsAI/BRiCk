...
Module Type PTRS_MIXIN (Import P : PTRS_INTF_MINIMAL).
  ...
  *)
  ...
  Section implicit_type_off.
    ...
    Axiom ptr_common_prefix : ∀ {σ p1 p2 o1 o2},
      p1 ,, o1 = p2 ,, o2 ->
      ptr_cong σ p1 p2.
    (* Argh, not true!
    TODO: strengthen
  ...
  Proof.
    ...
    - intros p p' p''
             ...
             [p'''' [o1' [o2' [Hp'' [Hp''' Hos']]]]].
      subst.
      destruct (ptr_common_prefix Hp'') as (p0 & oa & ob & ? & ? & Hos''); subst.
      exists p0.
      exists (oa ,, o1).
      exists (ob ,, o2').
      rewrite !offset_ptr_dot.
      intuition.
      rewrite /offset_cong !eval_offset_dot in Hos Hos' Hos'' |- *.
      rewrite {}Hos''; f_equiv.
      rewrite Hos -Hos'.
      move: Hos Hos' Hos''.
      rewrite /offset_cong !eval_offset_dot => Hos Hos' Hos''.
      -> <- <-.
      move: (Hp'').
      rewrite -!offset_ptr_dot.
