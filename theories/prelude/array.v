...
Instance Op_max_length : ZifyClasses.CstOp max_length :=
...
Section EqDec.
  ...
  #[program]
  Fixpoint eq_dec_partial a1 a2 n {struct n} :
    {(forall m, (m < n)%nat -> get a1 (of_Z (Z.of_nat m)) = get a2 (of_Z (Z.of_nat m)))} + {a1 <> a2} :=
    match n with
    | O => left _
    | S n =>
  ...
  Next Obligation.
    ...
    + intros i.
      ...
      specialize (H (Z.to_nat (to_Z i))).
      rewrite <- E in H.
      apply H.
      apply Znat.Z2Nat.inj_lt.
      * pose proof (to_Z_bounded i). now auto.
      * pose proof (to_Z_bounded (length a1)). now auto.
      * now auto.
    + now auto.
  Qed.
  Next Obligation.
...
Section Countable.
  ...
  #[global,program] Instance smallstr_countable : Countable (array A) := inj_countable to_list of_list _.
  Next Obligation.
    intros a.
    rewrite to_list_P.
    unfold of_list.
    ...
    - intros i.
      ...
      { rewrite map_length rev_length intseq_length. lia. }
