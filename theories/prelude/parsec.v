...
Section parsec.
  ...
  #[global] Instance MRet_M : MRet M := @stateT.ret bs _ _.
  #[global] Instance FMap_M : FMap M := @stateT.map bs _ _.
  #[global] Instance MBind_M : MBind M := @stateT.bind bs _ _.
  ...
  Definition any : M Byte.byte :=
    let* l := stateT.get in
    match l with
    | BS.EmptyString => mfail
    | BS.String b bs => const b <$> stateT.put bs
    end.
  ...
  Fixpoint exact_ (b : bs) (input : bs) {struct b} : optionT.M F (UTypes.prod bs unit) :=
    match b with
    | BS.EmptyString => mret (UTypes.pair input tt)
    | BS.String x xs =>
        match input with
        | BS.String y ys =>
