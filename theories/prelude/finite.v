...
Module invert_of_N.
  Section of_N.
    ...
    Definition of_N (r : N) : option A := head $ preimage to_N r.

    Lemma of_to_N `[Hinj : !Inj eq eq to_N] x : of_N (to_N x) = Some x.
    Proof. by rewrite /of_N preimage_inj_singleton. Qed.

    Lemma to_of_N (n : N) (x : A) :
      of_N n = Some x → to_N x = n.
    Proof. rewrite /of_N => Hof. by apply elem_of_preimage, head_Some_elem_of. Qed.
  End of_N.
End invert_of_N.
