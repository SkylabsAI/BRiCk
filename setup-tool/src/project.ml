...
let discover : unit -> (Filepath.t * pkg) list * (Filepath.t * config) list =
    fun _ ->
  ...
  let skip_file ~path config =
    ...
    List.exists (Filepath.equal path) config.proj_ignored
  in
  let pkgs = ref [(".", {pkg_name = config.coq_package})] in
  let add_package dir br_config =
    match br_config.brc_coq_package with None -> () | Some(pkg_name) ->
    pkgs := (dir, {pkg_name}) :: !pkgs
  in
  let chdir ~dir ~base config =
    ...
    add_package full_dir br_config;
