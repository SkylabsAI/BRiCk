...
(** [discover ()] returns a pair [(packages, configs)], where [packages] holds
    the list of packages defined in the project. For each such package, a pair
    [(dir, p)] is given, where [dir] is a path to the package's directory from
    the root, and [p] is the package metadata. The [configs] value lists every
    coevered C++ source file, together with the appropriate configuration. The
    function takes the project configuration, so ignored files/directories are
    not traversed. In case of failure, the program is aborted. *)
val discover : unit -> (Filepath.t * pkg) list * (Filepath.t * config) list
